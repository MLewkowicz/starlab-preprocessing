...
def inference(video_path, input_tag, interval, progress=gr.Progress()):
    ...
    # dense caption
    dense_caption = []
    dense_index = np.arange(0, len(data)-1, interval)
    ...
    # print(' | '.join(tag_1),' | '.join(tag_2), frame_caption, dense_caption, synth_caption[0], prediction)
    # return ' | '.join(tag_1),' | '.join(tag_2), frame_caption, dense_caption, synth_caption[0], prediction
    return [dense_caption, synth_caption[0], prediction]
...
interval_length = 10
start_time = 0
timecoded_transcript = format_transcript("oYMAX90kNkU.en.vtt", "output.txt")
with open("oYMAX90kNkU_10.txt", 'w') as f:
    for filename in os.listdir('clips10'):
        end_time=start_time + interval_length
        start = Timecode.convert_sec_to_timecode(start_time)
        end = Timecode.convert_sec_to_timecode(end_time)
        dirname = 'clips10/'
        video_path = dirname + filename
        dense_caption, synth_caption, prediction = inference(video_path, 'none', interval_length)
        transcript = extract_range(Timecode(start), Timecode(end), timecoded_transcript)
        interval_metadata = {"start": start, "end": end, "dense_caption": dense_caption, "synth_caption": synth_caption, "action_pred": prediction, "transcript": transcript['text']}
