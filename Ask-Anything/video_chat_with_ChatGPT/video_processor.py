...
class VideoProcessor:
    ...
    def process_video(self, video_link):
        ...
        if (video_link not in video_library):
            video_library[video_link] = self.download_video(video_link)
        ...
        transcript = self.process_transcript(subtitles)
        return transcript, metadata

    def process_transcript(self, subtitles):
        transcript = []
...
def main():
    video_file = "https://www.youtube.com/watch?v=oYMAX90kNkU"
    video_processor = VideoProcessor(video_file)
    transcript, metadata = video_processor.process_video(video_file)
    print(transcript)
