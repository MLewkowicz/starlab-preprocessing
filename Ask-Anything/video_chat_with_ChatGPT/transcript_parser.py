...
def format_altered_transcript(transcript_file, output_file):
    output = ""
    with open(transcript_file, 'r', encoding='utf-8') as f:
        ...
        for group in line_groups:
            ...
            segment = "".join(group.split("\n")[1:])
            pattern = r"(\d{2}:\d{2}:\d{2}\.\d{3}) --> (\d{2}:\d{2}:\d{2}\.\d{3})"
            match = re.search(pattern, first_line)
            if match:
                start_timecode = match.group(1)
                end_timecode = match.group(2)
                TIMECODED_TRANSCRIPT.append({"start": Timecode(start_timecode), "end": Timecode(end_timecode), "text": segment})
